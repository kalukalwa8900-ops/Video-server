...
// ================================
// Create Segment (MP4 - no mismatch)
// ================================
...
function createSegment({ imagePath, audioPath, text, duration, outPath, jobId, idx }) {
  return new Promise((resolve, reject) => {
    ...
    cmd
      ...
      .on("end", () => {
        console.log(`[seg${idx}] Segment completed`);
        resolve(outPath);
      })
      .on("error", (err, stdout, stderr) => {
        console.error(`[seg${idx}] ERROR:`, err.message);
        console.error(`[seg${idx}] stderr:`, stderr);
        reject(err);
      })
      .run();
  });
...
// ================================
// Concat Segments → Final WebM
// ================================

function concatSegments(segPaths, outPath) {
  return new Promise((resolve, reject) => {

    // list file extension is always .txt regardless of output format
    const listPath = outPath.replace(/\.[^.]+$/, "_list.txt");
    ...
    ffmpeg()
      ...
      .output(outPath)
      .on("start", () => {
        console.log(`[concat] Starting video concat (WebM)...`);
      })
      .on("progress", (progress) => {
      ...
      .on("end", () => {
        ...
        console.log(`[concat] Video concat completed`);
        resolve(outPath);
      })
      .on("error", (err, stdout, stderr) => {
...
app.post("/render", (req, res, next) => {
  ...
  return diskUpload.array("images", 30)(req, res, () =>
    renderFromMultipart(req, res)
  );
});
...
async function renderFromProject(req, res) {
  ...
  if (!panels.length) {
    return res.status(400).json({
      success: false,
      error: "No complete panels found to render"
    });
  }
  ...
  try {
    console.log(`[${jobId}] Rendering ${panels.length} panels...`);

    for (let i = 0; i < panels.length; i++) {
      const p = panels[i];

      // Segments are MP4 (H.264) — no codec/container mismatch
    ...
    const finalPath = path.join(OUTPUT_ROOT, `${jobId}_final.webm`);
    await concatSegments(segPaths, finalPath);
    cleanupFiles(segPaths);

    const url = `${req.protocol}://${req.get("host")}/output/${jobId}_final.webm`;
    ...
    return res.json({
      ...
      device_support: "Android, Chrome, Firefox, Edge"
    });
